...
def nonceFinder(wallet_list, miner_public):
    ...
    while not break_now:
        ...
        placeholder.add_output(miner_public,25.0) #tx with just outputs 
        newBlock.addTx(placeholder)
        #TODO sort tx_list by tx per byte 
        for tx in tx_list:
            newBlock.addTx(tx)
        ...
        if verbose: print ("new block has " + str(len(newBlock.data)) + " txs.") # prints num txs so we can watch in real-time
